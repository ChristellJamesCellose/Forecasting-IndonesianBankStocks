...
# def plot_data(model_type, data, result):

#st.subheader('Number of Months')
#months = st.slider('Choose Number of Months that will be predicted', min_value = 0.0, max_value = 10.0, step = 0.5)
...
def main():       
    ...
    <h1 style ="color:black;text-align:center;">Forecasting Indonesian Bank Stock Price App</h1> 
    </div> 
    """
    ...
    bank_type = st.selectbox('Input Bank Stock',("BBCA", "BBNI", "BBRI")) 
    input_type = st.selectbox('Input Stock Price',("Open","High","Low","Volume","Adj Close","Close")) 
    ...
    if st.button("Predict"): 
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=data2.iloc[0:2004].index, y=data2[input_type].iloc[0:2004], name = 'Data'))
        fig.add_trace(go.Scatter(x=data2.iloc[2004:].index, y=data2["predicted_mean"].iloc[2004:], name = 'Forecast'))
        fig.layout.update(title_text=bank_type + " " + input_type, xaxis_rangeslider_visible=True)
        st.plotly_chart(fig)
      
  #  data2 = data[[input_type]]
  #  data2 = data2.append(result)
    
   # plot_data(input_type, data, result)
...
# pipreqs C:\Users\user\ADS\Final Project\Modelling\app.py
